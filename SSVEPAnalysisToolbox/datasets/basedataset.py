...
class BaseDataset(metaclass=abc.ABCMeta):
    ...
    def default_ref_sig_fun(self, dataself, sig_len: float, N: int, phases: List[float]):
        ...
        ref_sig = [gen_ref_sin(freq, dataself.srate, L, N, phase) for freq, phase in
                   zip(dataself.stim_info['freqs'], phases)]
        return ref_sig
        ...
        for subject in self.subjects:
            ...
            while (not download_flag) and (download_try_count < total_retry_time):
                if download_try_count > 0:
                    warnings.warn(
                        "There is an error when donwloading '{:s}' data. So retry ({:n}) after 10 seconds.".format(
                            subject.ID, download_try_count))
                    time.sleep(10)
                ...
                for download_flag, source_url, desertation in zip(download_flag_store, source_url_store,
                                                                  desertation_store):
        ...
        if self.support_files is not None and self.path_support_file is not None:
            for file_name in self.support_files:
                ...
                while (not download_flag) and (download_try_count < total_retry_time):
                    if download_try_count > 0:
                        warnings.warn(
                            "There is an error when donwloading '{:s}'. So retry ({:n}) after 10 seconds.".format(
                                file_name, download_try_count))
                        time.sleep(10)
    ...
    def get_data_single_trial(self,
        ...
        if sig_len + t_latency + self.t_prestim > self.trial_len:
            ...
            warnings.warn('Total time length ({:n}) is larger than single trial time ({:n})'.format(sig_len + t_latency,
                                                                                                    self.trial_len))
        ...
        eeg_total = eeg_total[:, start_t_idx:end_t_idx]
        ...
        if len(eeg_total.shape) == 1:
            ...
            raise ValueError(
                "EEG shape has error. Please check the output of the filterbank function should have three dimention. The 1st dimention is the filterbank number")
    ...
    def get_data_trial(self,
                       ...
                       sig_len: float,
                       t_latency: float):
        return [self.get_data_single_trial(sub_data, block_idx, trial_idx, channels, sig_len, t_latency) for trial_idx
                in trials]
    ...
    def get_phase_single_trial(self,
        ...
        X_fft = X_fft[0][filter_bank_idx, :, :]
        Y_fft = Y_fft[0]
        phase = freqs_phase(X_fft, self.stim_info['freqs'][Y_fft], self.stim_info['phases'][Y_fft],
                            srate,
                            detrend_flag=detrend_flag,
                            NFFT=NFFT)
        if remove_target_phase:
            phase -= self.stim_info['phases'][Y_fft]
        return phase
    ...
    def get_snr(self,
        ...
        if type.lower() == 'fft':
            snr = np.zeros((len(self.subjects), self.block_num, self.trial_num,
                            len(self.channels)))  # subj * block_num * stimulus_num * ch_num
        elif type.lower() == 'sine':
        ...
        for sub_idx in range(len(self.subjects)):
            for block_idx in range(self.block_num):
                if display_progress:
                    pbar.update(1)
                X_all_trials, Y_all_trials = self.get_data_all_trials(sub_idx, [block_idx],
                                                                      list(range(len(self.channels))), sig_len,
                                                                      t_latency)
                for trial_idx, X_single_trial in enumerate(X_all_trials):
                    ...
                    if type.lower() == 'fft':
                        for ch_idx in range(len(self.channels)):
                            ...
                            snr[sub_idx, block_idx, trial_idx, ch_idx] = freqs_snr(X_fft,
                                                                                   self.stim_info['freqs'][Y_fft],
                                                                                   srate, Nh,
                                                                                   detrend_flag=detrend_flag,
                                                                                   NFFT=NFFT)
                    elif type.lower() == 'sine':
    ...
    def get_phase(self,
        ...
        if sig_len is None:
            sig_len = self.trial_len
        phase = np.zeros((len(self.subjects), self.block_num, self.trial_num,
                          len(self.channels)))  # subj * block_num * stimulus_num * ch_num
        if remove_pre_and_latency:
            sig_len = sig_len - self.t_prestim - t_latency
        if remove_break:
        ...
        for sub_idx in range(len(self.subjects)):
            for block_idx in range(self.block_num):
                ...
                for trial_idx, X_single_trial in enumerate(X_all_trials):
                    ...
                    for ch_idx in range(len(self.channels)):
                        X_fft = X_single_trial[filter_bank_idx, ch_idx:(ch_idx + 1), :]
                        phase[sub_idx, block_idx, trial_idx, ch_idx] = freqs_phase(X_fft,
                                                                                   self.stim_info['freqs'][Y_fft],
                                                                                   self.stim_info['phases'][Y_fft],
                                                                                   srate,
                                                                                   detrend_flag=detrend_flag,
                                                                                   NFFT=NFFT)
                        if remove_target_phase:
                            phase[sub_idx, block_idx, trial_idx, ch_idx] -= self.stim_info['phases'][Y_fft]
        return phase
